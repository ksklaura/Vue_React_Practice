...
function TodoInput({ callbackAddTodo }) {
  ...
  const [isShowModal, setIsShowModal] = useState(true); // 상태값이 기본타입인 경우
  // true이면 경고문이 나오고, false면 경고문이 나오지 않음!
  ...
  const handlerShowModal = (e) => {
    ...
    // isShowModal의 useState를 false로 바꿔야 경고문이 없어짐.
  ...
  const handlerAddTodo = (e) => {
    ...
    // input 태그에 빈 문자열이나 공백이 입력 되는 경우는 modal 창이 출력되게 하기.
    if (
      // 아랫줄은 value === null || value === undefined || value === "" 와 동일한 의미
      !value ||
      !value.trim()
    ) {
      // isShowModal = true;
    ...
    // 부모 컴포넌트인 TodoContainer 의 callbackAddTodo 메서드 호출
    callbackAddTodo(value);

    // add 후에 input 태그의 입력 값 지우기.
    // input의 value 속성에 값을 설정하기. ref이름.current.속성명
    // value 속성값 가져오기. ref이름.current.value
